...
We construct these sets by simply filtering out counterfactuals that are given the same labels as their original instances during the crowd labeling (40\%--63\% depending on the task).
...
We also evaluate model capabilities with CheckList tests (\cite{checklist:acl20}, available for \sst and \qqp).
...
On \nli, \sysname counterfactuals are as effective or more effective than the ones created from scratch (\mcad).
\maug also improves on CheckList tests that previously had high error rates: 
It significantly lowers the error rates on 11 out of 27 \qqp tests\footnote{The absolute error rate drops for at least $5$ points, with a relative difference of more than $10\%$.}, and only increasing it in $2/27$ tests).
For \sst, it improves on 5 out of 15, hurting 1.
