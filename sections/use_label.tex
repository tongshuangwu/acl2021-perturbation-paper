...
\vspace{-5pt}
\caption{
Sample CheckList tests for \qqp, with $\Delta$ fail\% denoting the failure rate differences between \maug and \mcomp (negative means \maug is better).
...
RoBERTa fintuned on MNLI~\cite{williams-etal-2018-broad} for \nli\footnote{\url{https://huggingface.co/roberta-large-mnli}},
...
Though the counterfactuals remains useful on most datasets, it appears too many counterfactuals may be harmful (\eg Amzbook when $m=n=2k$). \wts{Maybe cut/appendix?}
...
Interestingly, the $m/n$ ratio seems important.
As in Figure~\ref{fig:sst_trend}, while the counterfactual remains effective on most datasets, it hurts model performance on Amzbook when the counterfactual takes a large proportion (Yelp followed a similar but more mild trend).
We suspect that flipping out too much original data affect the data diversity, and in turn decreases the model performance.
...
Instead, we follow (but adjust) \citet{chen2019slice}'s data slicing strategies, and prioritize counterfactuals that can improve error cases identified in DNC~\cite{kim2019probing}.
As a result, \maug was able to perform better on DNC (Table~\ref{table:aug_nli}), as well as some other challenge sets~\cite{naik2018stress, glockner-etal-2018-breaking, wang2018glue}.
...
\paragraph{\qqp.}
We validate the models using tests defined in CheckList, a behavioral testing framework that measures models' linguistic capabilities using the failure rates of multiple tests.\footnote{Because failure rates are more sensitive, we say a model capability is affected, if the failure rate of a test changes more than 5\% (\eg 20\% to 21\% is insignificant), and the delta accounts for more than 10\% of the original failure rate (\eg decreasing 8\% from 100\% does not count.)}
...
With $n=20,000$ and $m=1,911$, \maug achieved a compatible in-domain accuracy, reduced the failure rates on 11 tests (out of the 27 where \mcomp failed $>20\%$), and increased that for 2 tests.
...
However, the model gets significantly better on  \texttt{more X $\not\eq$ less X} by sacrificing \texttt{more X $=$ less antonym(X)}, indicating potential overfitting.
