...
When changing one number to another (\swap{\texttt{NUM}}{\texttt{NUM}}), the model only flips the label in 64.7\% cases, which is abnormal --- we would expect all cases to be like in Figure~\ref{fig:err_analysis_two_three}A.
...
For example, \exinline{is \add{not} reading} can be translated into a set of templates $t$ that are as finer-grained as \swap{is reading}{is not reading}, or as sparse as \addplus{\texttt{PART}}.
Similarly, \exinline{are \add{not} playing} also results in \addplus{\texttt{PART}} or \addplus{\texttt{not}}, but not \swap{is reading}{is not reading}.
...
Meanwhile, to avoid overfitting to one instance (\eg extracting a template \swap{red}{\texttt{ADJ}} only because ``red'' is repeatedly perturbed in one $x$ 100 times), we prefer (2) templates that perturb various unique $x$.
