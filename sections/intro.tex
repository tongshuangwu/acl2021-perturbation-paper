...
Applications of counterfactual reasoning to Natural Language Processing (NLP) generally specify the relationship \relation{\xp}, and then create $\xp$ according to the relationship.
...
However, we observe that the generation methods do not have to be isolated, as various applications share similar requirements on \relation{\xp} (\eg minimal edits).
...
First, in generation, given an input $x$, we produce a set of counterfactuals $\hat{\xset} = \{\xp_1, \xp_2, ...\}$ with reasonable but \emph{application-agnostic} relationships $x \rightarrow \xp_i$ (Figure~\ref{fig:teaser}B).
...
As a result, \sysname learns to generate general purpose $\xp$ that are \emph{plausible}, \emph{diverse}, and \emph{close to $x$}.
...
We also demonstrate how \sysname can produce \emph{black-box counterfactual explanations} (\S\ref{sec:app_explain}).
...
\item Finetune GPT-2 into a generator called \sysname, by collecting paired sentences and extending prior work on infilling language models with \tagstrs. 
