...
As a result, they neglect prediction-preserving counterfactuals even though they are equally effective for revealing model behaviors, like the examples in Figure~\ref{fig:teaser}D.
...
Intrinsic evaluations show that \sysname generates $\xp$ that are \emph{fluent}, \emph{diverse}, and \emph{close to $x$}, and that the \emph{control} effectively retrieves perturbations that are rare to sample from off-the-shelf language models. %(\eg 42\% more negations).
