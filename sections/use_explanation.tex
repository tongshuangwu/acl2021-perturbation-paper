...
% seems like C is the example to highlight, no? 
...
Then, we select groups with unstable prediction changes, quantified as large entropy on $f(x) \veryshortarrow f(\xp)$ for its $\xp$.
For example, one global abnormality for the \nli RoBERTa model in \S\ref{subsec:contrast_set} is \swap{two}{three} in the hypothesis.
Out of the 253 $\xp$ whose original $f(x)=$ \emph{entailment}, $138$ flipped to \emph{contradiction}, $22$ to \emph{neutral}, yet $93$ was intact, resulting in entropy $I=0.91$.
