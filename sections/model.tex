...
For example, \swap{great}{not great}, \swap{kids}{no one} in Figure~\ref{fig:teaser} both instantiate for \ctrltag{negation}.\footnote{All examples shown are actual generations of the model. \wts{Move this footnote to where it first occurs.}}
...
For each \tagstr, we compare the \emph{control success rate} of \sysname and \sysname-a on \tofix{120} prompts (\tofix{50} unique original sentences).
...
The success rate increases by \tofix{$48.7\% \pm 38.1\%$} across all \tagstrs.
\tagstr{lexical} has the smallest increment \tofix{from $93\%$ to $100\%$}, mostly because \emph{\sysname-a} tend to frequently replace words.
\tagstr{insert} is the most impactful \tagstrshort (from \tofix{$12\%$ to $100\%$}) --- \sysname-a rarely insert clues on its own.
...
(1) The dual manipulation from the \tagstrs and the blanks can conflict, \eg \exinline{a dog is embraced by a [BLANK]} would not respond to \ctrltag{negation}.
...
For example, the model tends to perturb the quantifier ``two'' in \exinline{two dogs are running}, regardless of the \tagstrshort.
