...
\tofix{We use a pair of two sentences interchangeably as $x$ and $\xp$ to learn the \tagstrs both ways.}
To balance the distribution (Table~\ref{table:gpt_train_stats} in \tofix{Appendix~\ref{TODO}}), for each dataset, we extract \tagstrs from all the $(x, \xp)$, and randomly sample up to 10,000 instances per \tagstrshorts.
