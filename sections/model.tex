...
Each $\xp_i$ perturbs $x$ with certain strategies $s$ ($s(x, \xp_i)=$ negations, syntactic restructuring, etc.), and the edited spans --- including a set of tokens removed from $x$ (denoted as $r(x, \xp_i)$) and added to $\xp$ ($a(x, \xp_i)$), and the affected parsing tree structures $t(\xp)$ --- are instantiations of the strategies.
...
We further modify the training prompts to allow targeted counterfactuals.
Such controls boosts the diversity on top of LMs.
...
\emph{Where-to-perturb, with \BLANK}.
To perturb specific parts of $x$, We borrow the blank fill-in structure~\cite{donahue2020enabling}, \ie targeted subphrases are replaced with special \BLANK tokens, and the actual content (``answers'' to blanks) are concatenated to the end. 
To allow flexible blanking at the generation time, instead of merely blanking the edited spans, during training, we also extend the blank to cover their associated parsing structures.
...
We further maximize the syntactic differences (\eg \ctrltag{resemantic}, \ctrltag{insert}, or \ctrltag{restructure}), such that the controls are more explicit to follow than semantic-oriented ones (synonyms, antonyms, paraphrases).
...
GPT-2 still does not always generate \emph{realistic} sentences, especially when the \tagstr and the \BLANK does not match (illustrated in \S\ref{subsec:intrinsic}), and therefore require additional filtering.
...
Rather, it (targetedly) generates a large amount of counterfactuals that should be further selected or ranked.
